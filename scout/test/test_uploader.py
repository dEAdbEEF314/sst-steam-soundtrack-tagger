...
def steam_app(tmp_path) -> SteamApp:
    ...
    return SteamApp(
        ...
        acf_path=str(acf),
        audio_files=[str(track1), str(track2)],
    )
...
def test_file_keys_prefix(steam_app):
    result = upload_app(None, "buckets", steam_app, dry_run=True, upload_audio=True)
    assert all(k.startswith("ingest/1234567/files/") for k in result.file_keys)
...
def test_audio_keys_are_relative_to_audio_root(steam_app_with_format_dir):
    result = upload_app(None, "buckets", steam_app_with_format_dir, dry_run=True, upload_audio=True)
    assert result.file_keys == [
        "ingest/1297600/files/01 - avalanche.flac",
        "ingest/1297600/files/02 - barriers.flac",
    ]


def test_scout_result_includes_location_and_format(steam_app_with_format_dir):
    mock_s3 = MagicMock()
    upload_app(mock_s3, "buckets", steam_app_with_format_dir, dry_run=False, upload_audio=False)

    call_kwargs = mock_s3.put_object.call_args.kwargs
    body = json.loads(call_kwargs["Body"].decode("utf-8"))
    assert body["storage_location"] == "music"
    assert body["format_dir"] == "art of rally OST - flac"
